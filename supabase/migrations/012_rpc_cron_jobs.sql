...
BEGIN
  ...
  v_job_id := enqueue_job('REFRESH_HOT_VIDEOS', jsonb_build_object(), 10, 'refresh_hot_videos:' || v_hour);
...
BEGIN
  ...
  v_job_id := enqueue_job('REFRESH_VIDEO_STATS', jsonb_build_object('mode', 'weekly'), 6, 'refresh_video_stats_weekly:' || v_week);
