<template>
  ...
  <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ t('categoriesPage.categoryNotFound') }}</h3>
      ...
      <div v-else-if="category">
        ...
        <div class="mb-8">
          ...
          <div class="flex items-center space-x-6 text-sm text-gray-500">
            <span>{{ totalVideos }} {{ t('categoriesPage.videos') }}</span>
          </div>
        </div>

        <!-- Sorting Controls -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-6">
          <h2 class="text-xl font-semibold">{{ t('categoriesPage.videosSection') }}</h2>
          <div class="flex items-center gap-2">
            ...
            >
              <option value="all">{{ t('categoriesPage.periods.all') }}</option>
              <option value="30">{{ t('categoriesPage.periods.30') }}</option>
              <option value="7">{{ t('categoriesPage.periods.7') }}</option>
            </select>
          </div>
        </div>
        ...
        <div v-else class="text-center py-12">
          ...
          <h3 class="text-lg font-medium text-gray-900 mb-2">{{ t('categoriesPage.emptyState.title') }}</h3>
          <p class="text-gray-600">{{ t('categoriesPage.emptyState.description') }}</p>
        </div>
        ...
        >
          <div v-if="isLoadingMore" class="flex items-center gap-2 text-muted dark:text-gray-400">
            ...
            <span class="text-sm">{{ t('categoriesPage.loading.moreVideos') }}</span>
          </div>
        </div>
      </div>
    </div>
